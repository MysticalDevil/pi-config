...
function scanProject(cwd: string): ProjectInfo {
	...
	for (const entry of entries) {
		...
		try {
			...
			if (stat.isDirectory()) {
				...
				files.push(entry);
				if (entry.startsWith(".") || entry.endsWith(".config.js") || entry.endsWith(".config.ts") ||
					entry.endsWith(".json") || entry.endsWith(".toml") || entry.endsWith(".yaml") ||
					entry.endsWith(".yml") || entry === "Makefile" || entry === "Dockerfile") {
	...
	for (const [lang, sigs] of Object.entries(LANGUAGE_SIGNATURES)) {
		for (const sig of sigs) {
			if (sig.startsWith("*.")) {
				if (findFiles(cwd, sig)) { languages.push(lang); break; }
			} else if (files.includes(sig) || configFiles.includes(sig)) {
				languages.push(lang); break;
			}
		}
	}
	...
	if (files.includes("pnpm-lock.yaml") || configFiles.includes("pnpm-workspace.yaml")) packageManager = "pnpm";
...
function generateTree(dir: string, prefix: string, maxDepth: number, currentDepth: number = 0): string {
	...
	entries = entries
		.filter(e => {
			if (e.startsWith(".") && e !== ".env.example" && e !== ".gitignore" && e !== ".github") return false;
				...
				await ctx.ui.custom((_tui, theme, _kb, done) => {
					...
					return {
						render(width: number) {
							...
							result.push("");
							result.push(theme.fg("dim", "Use /init --force to write this file. Press Esc to close."));
							return result;
						},
						invalidate() {},

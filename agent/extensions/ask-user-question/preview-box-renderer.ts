/**
 * preview-box-renderer.ts — Bordered box composition and fence-marker stripping
 * for markdown preview blocks.
 *
 * Ported from @juicesharp/rpiv-ask-user-question view/components/preview/preview-box-renderer.ts
 */
 ...
 * code blocks; this strip leaves only the highlighted code body.
 */
export function stripFenceMarkers(lines: readonly string[]): string[] {
  return lines.filter((line) => {
    const clean = line.replace(ANSI_SGR_RE, "");
 ...
 * When `hidden > 0`, the bottom-row dash run is replaced with
 * ` ✂ ── N lines hidden ── `.
 */

/**
 * markdown-content-cache.ts — Per-question cache for rendered markdown previews.
 *
 * Width-keyed: switching the inner width invalidates every cached Markdown's render
 * output (pi-tui's Markdown.render(width) re-wraps when width changes).
 *
 * Ported from @juicesharp/rpiv-ask-user-question view/components/preview/markdown-content-cache.ts
 */
 ...
 * lazy on first request, invalidated on width change.
 */
export class MarkdownContentCache {
  ...
  /**
   * Compute the body lines for a given option at a given inner width.
   * Width changes invalidate the per-Markdown render cache.
   */

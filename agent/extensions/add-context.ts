...
function collectFiles(cwd: string, targetPath: string, depth: number, files: Map<string, true>): void {
...
function walkDir(dir: string, maxDepth: number, files: Map<string, true>, currentDepth: number = 0): void {
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		for (const entry of entries) {
			if (entry.type === "custom" && entry.customType === "dynamic-context") {
				const data = entry.data as { entries: Array<{ path: string; type: "file" | "directory"; addedAt: number }> } | undefined;
				if (data?.entries) {
	...
	pi.registerCommand("add-dir", {
		...
		handler: async (args, ctx) => {
			...
			ctx.ui.notify(
				`Added ${added.length} file(s) from "${targetPath}" (depth ${depth})`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("context", {
		...
		handler: async (args, ctx) => {
			...
			if (contextEntries.length === 0) {
				ctx.ui.notify("No dynamically added context. Use /add-dir or /add-context to add files.", "info");
				return;
			}

	...
	pi.registerCommand("cost", {
		...
		handler: async (args, ctx) => {
			...
			if (tokens.includes("--watch")) {
				...
				ctx.ui.notify(
					showWidget ? "Cost widget enabled (footer)" : "Cost widget disabled",
					"info",
				);
				updateWidget(ctx);
				return;
			...
			lines.push(`  Total:     ${formatTokens(stats.totalInput + stats.totalOutput).padStart(8)} tokens`);
			...
			if (ctxLimit > 0) {
				...
				const color = ctxTokens / ctxLimit > 0.8 ? "warning" : ctxTokens / ctxLimit > 0.5 ? "accent" : "muted";
				lines.push(ctx.ui.theme.bold("Context Window:"));
				lines.push(`  ${ctx.ui.theme.fg(color, bar)} ${formatTokens(ctxTokens)}/${formatTokens(ctxLimit)} (${pct}%)`);
				lines.push("");
			}
			...
			if (detail && stats.turns.length > 0) {
				...
				lines.push("");
				lines.push(`  ${"Turn".padEnd(6)} ${"Input".padStart(8)} ${"Output".padStart(8)} ${"Cost".padStart(8)} ${"Model"}`);
				lines.push(`  ${"────".padEnd(6)} ${"─────".padStart(8)} ${"──────".padStart(8)} ${"────".padStart(8)} ${"─────"}`);

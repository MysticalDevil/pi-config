...
function toggleExtension(cwd: string, name: string, enable: boolean): string | null {
  const dirs = [
...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("extensions", {
    description: "List, enable, or disable extensions (usage: /extensions [enable|disable] <name>)",
    handler: async (args, ctx) => {
      ...
      if (!name) {
        ctx.ui.notify("Usage: /extensions enable <name> | /extensions disable <name>", "error");
        return;
      }
      ...
      ctx.ui.notify(`Extension "${name}" ${result}. Run /reload to apply.`, "info");
    },
  });
}

...
export default function (pi: ExtensionAPI) {
	...
	async function getChanges(cwd: string): Promise<FileChange[]> {
		// Get git diff stats
		const { stdout, code } = await pi.exec("git", ["diff", "--stat", "HEAD"], { cwd });
		...
		for (const line of lines) {
			...
			if (!fs.existsSync(fullPath)) status = "deleted";
				...
				const { stdout: logOut } = await pi.exec("git", ["log", "--oneline", "--", file], { cwd });
				if (logOut.trim() === "") {
					const { stdout: stagedOut } = await pi.exec("git", ["ls-files", "--", file], { cwd });
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			if (tokens[0] === "--commit") {
				...
				await pi.exec("git", ["commit", "-m", `[pi-review] accept ${files.length} file(s)`], { cwd: ctx.cwd });
			...
			for (const change of state.changes) {
				...
				const decision = await new Promise<string>((resolve) => {
					ctx.ui.custom((_tui, theme, _kb, done) => {
						...
						if (diff.split("\n").length > 30) {
							lines.push(theme.fg("dim", `... ${diff.split("\n").length - 30} more lines`));
						}
						...
						return {
							...
							handleInput(data: string) {
								if (data === "a" || data === "A") { done(undefined); resolve("accept"); }
								else if (data === "r" || data === "R") { done(undefined); resolve("reject"); }
								else if (data === "v" || data === "V") { done(undefined); resolve("view"); }
								else if (data === "q" || data === "Q" || data === "\x1b") { done(undefined); resolve("quit"); }
							},
						};
					}).catch(() => resolve("quit"));
				});
				...
				if (decision === "accept") {
					...
					// Revert the file
					await pi.exec("git", ["checkout", "--", change.file], { cwd: ctx.cwd });

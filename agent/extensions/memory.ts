...
const MemoryParams = Type.Object({
  ...
  text: Type.Optional(
    Type.String({ description: "Memory text (for save) or search query (for recall)" }),
  ),
  id: Type.Optional(Type.String({ description: "Memory ID (for remove)" })),
  tags: Type.Optional(
    Type.Array(Type.String(), { description: "Tags for categorizing (for save)" }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Memory",
    description:
      "Manage persistent cross-session memories. Actions: save (store a fact), recall (search), list (show all), remove (delete by id), clear (remove all).",
          ...
          if (results.length === 0) {
            return {
              content: [
                {
                  type: "text",
                  text: query ? `No memories matching "${query}".` : "No memories saved yet.",
                },
              ],
        ...
        default:
          return {
            content: [
              {
                type: "text",
                text: `Unknown action: ${params.action}. Use: save, recall, list, remove, clear.`,
              },
            ],
            details: {},
          };
  ...
  pi.registerCommand("memory", {
    description:
      "Manage persistent memories (usage: /memory [add <text>|search <q>|remove <id>|clear|list])",
      ...
      switch (subcommand) {
        ...
        case "clear": {
          const ok = await ctx.ui.confirm(
            "Clear all memories?",
            `Delete all ${memories.length} saved memories?`,
          );
          if (!ok) return;
          saveMemories([]);
        ...
        default: {
          if (memories.length === 0) {
            ctx.ui.notify(
              "No memories saved. The agent can save memories using the memory tool.",
              "info",
            );
            return;
          }

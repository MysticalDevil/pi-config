...
function getSwatchColors(themeFile: string): Record<string, string> | null {
	// Try global first
	const globalPath = path.join(process.env.HOME || "~", ".pi", "agent", "themes", `${themeFile}.json`);
...
function colorValue(val: string | number | undefined, vars: Record<string, string> | undefined): string {
		...
		handler: async (args, ctx) => {
			...
			if (subcommand && subcommand !== "--list" && subcommand !== "--current") {
				const match = themes.find((t) =>
					t.file === subcommand ||
					t.name.toLowerCase().includes(subcommand.toLowerCase())
				);
				if (!match) {
					ctx.ui.notify(`Theme "${subcommand}" not found. Use /theme --list to see available themes.`, "warning");
					return;
				}
					...
					for (let i = 0; i < visibleThemes.length; i++) {
						...
						if (swatch) {
							...
							const swatchBar = [
								...
								swatch.accent ? ` ${swatch.accent.startsWith("#") ? "\x1b[38;2;" + hexToRgb(swatch.accent).join(";") + "m●\x1b[0m" : "●"} ` : "",
								swatch.text ? ` ${theme.fg("dim", "T")} ` : "",
							].filter(Boolean).join("");

							const name = isSelected
								? theme.fg("accent", theme.bold(t.name))
								: t.name;
							const icon = t.isDark ? "🌙" : "☀️";

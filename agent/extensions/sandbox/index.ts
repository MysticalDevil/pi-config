				...
				const onAbort = () => {
					if (child.pid) {
						try {
							...
							console.error(
								"sandbox: abort failed to kill process group, direct kill:",
								e,
							);
							child.kill("SIGKILL");
						}
...
function scanGitDiff(diff: string): DiffFinding[] {
	...
	const patterns: Array<{ pattern: RegExp; type: string }> = [
		...
		{
			pattern:
				/([a-zA-Z0-9_]+)\s*=\s*["']?(github_pat_[a-zA-Z0-9_]{30,})["']?/g,
			type: "GitHub token",
		},
		{
		...
		{
			pattern:
				/([a-zA-Z0-9_]+)\s*[:=]\s*["']?(mongodb(?:\+srv)?:\/\/[^:]+:[^@]+@)/g,
			type: "DB connection",
		},
		{
			pattern:
				/([a-zA-Z0-9_]+)\s*[:=]\s*["']?(postgres(?:ql)?:\/\/[^:]+:[^@]+@)/g,
			type: "DB connection",
		},
		{
	...
	for (const line of lines) {
		...
		for (const { pattern, type } of patterns) {
			...
			while (match !== null) {
				...
				const masked =
					captured.length > 25
						? `${captured.slice(0, 12)}...${captured.slice(-5)}`
						: "***";
...
export default function (pi: ExtensionAPI) {
	...
	function applyMode(
		...
		currentMode = mode;
		sandboxActive =
			mode === "sandbox" && !!bwrapAvailable && sandboxConfig.enabled;
		...
		if (ctx?.ui) {
			ctx.ui.setStatus(
				"permissions",
				ctx.ui.theme.fg(color, `${emoji} Permissions: ${mode}`),
			);
	...
	pi.on("user_bash", (event, ctx) => {
		...
		if (
			...
			!/\bgit\s+commit\s*--dry-run\b/.test(command)
		) {
			try {
				...
				if (
					...
					ctx.ui.notify(
						`Secret-detection scan failed: ${e.message}`,
						"warning",
					);
				}
			}
		...
		if (currentMode === "auto-review") {
			...
			if (evaluation.decision === "prompt") {
				...
				if (turnApproved) {
					...
					const justification =
						evaluation.matchedRules[0]?.justification ?? "requires review";
					const preview =
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		for (const entry of ctx.sessionManager.getEntries()) {
			if (entry.type === "custom" && entry.customType === "permissions-mode") {
				...
				if (
					data?.mode &&
					["sandbox", "auto-review", "full-access"].includes(data.mode)
				) {
		...
		if (sandboxActive || initialMode === "sandbox") {
			const writePathCount = sandboxConfig.writablePaths.filter((p) =>
				existsSync(p),
			).length;
			const denyPathCount = sandboxConfig.deniedPaths.filter((p) =>
				existsSync(p),
			).length;
			ctx.ui.setStatus(
				"sandbox-detail",
				ctx.ui.theme.fg(
					"dim",
					`${writePathCount} writable, ${denyPathCount} denied`,
				),
			);
		}
	});
	...
	pi.on("session_shutdown", async () => {
		...
		try {
			...
			if (
				...
				console.error(
					"sandbox: failed to clean up /tmp/.bwrap-empty:",
					e.message,
				);
			}
		}
	...
	pi.registerCommand("permissions", {
		description:
			"Switch permission mode: sandbox (default), auto-review, or full-access",
	...
	pi.registerCommand("sandbox", {
		...
		handler: async (_args, ctx) => {
			...
			const lines = [
				...
				"",
				"Writable paths:",
				...config.writablePaths.map(
					(p) => `  ${existsSync(p) ? "✅" : "⚠️ "} ${p}`,
				),
				"",
				"Denied paths (hidden):",
				...config.deniedPaths.map(
					(p) => `  ${existsSync(p) ? "🔒" : "  "} ${p}`,
				),
				"",
				...
				...hooks
					...
					.map(
						(h) =>
							`  ${h.enabled ? "✅" : "❌"} [${h.type}] ${h.name} (prio ${h.priority})`,
					),
			];
